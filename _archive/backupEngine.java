...
public class Engine implements EngineService, RequireDataService{
  private static final double friction=HardCodedParameters.friction,
                              ...
                              maxWidth=HardCodedParameters.maxWidth,
                              maxHeight=HardCodedParameters.maxHeight;
  ...
  private void updateCommandHeroes(){
    ...
    if (moveRight){
        // System.out.println(heroesVX+heroesStep);
        // System.out.println(heroesVX+heroesStep <= maxWidth);
        // if(heroesVX+heroesStep <= maxWidth){
        //     System.out.println(heroesVX+heroesStep);
        //     heroesVX+=heroesStep;
        // }
        heroesVX+=heroesStep;
  ...
  private void updatePositionHeroes(){
    ...
    if (data.getHeroesPosition().x<0){
        data.setHeroesPosition(new Position(0,data.getHeroesPosition().y));
    }
    if (data.getHeroesPosition().x>maxWidth){
        data.setHeroesPosition(new Position(maxWidth,data.getHeroesPosition().y));
    }
    if (data.getHeroesPosition().y<0){
        data.setHeroesPosition(new Position(data.getHeroesPosition().x,0));
    }
    if (data.getHeroesPosition().y>maxHeight){
        data.setHeroesPosition(new Position(data.getHeroesPosition().x,maxHeight));
    }
  }
